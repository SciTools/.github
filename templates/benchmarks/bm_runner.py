...
def _prep_data_gen_env() -> None:
    ...
    if data_gen_var in environ:
        ...
        env_setup_commands: list[str] = ["NOT", "TEMPLATED"]
        _subprocess_runner(env_setup_commands)
        env_directory: Path = Path("NOT/TEMPLATED")
...
def _setup_common() -> None:
    ...
    # ADD OTHER REPO-SPECIFIC CHECKS HERE, E.G. NOX.
...
class _SubParserGenerator(ABC):
    ...
    def func(args: argparse.Namespace):
        """The operation(s) which the subparser performs.

        Called once the subparser has been parsed.
        """
        _ = args
...
class Overnight(_SubParserGenerator):
    ...
    description = (
        ...
        "comparing each to its parent for performance shifts.\n"
        "Designed for checking the previous 24 hours' commits, typically in a "
        "scheduled script.\n"
        "Uses `asv run`.\n"
        # ADD NOTE FOR HOW THIS RELATES TO REPO-SPECIFIC GHA WORKFLOWS.
        #  e.g. in GHA performance shifts will be reported in a new issue.
    )
    epilog = (
...
class Branch(_SubParserGenerator):
    ...
    description = (
        ...
        "Uses `asv run`."
        # ADD NOTE FOR HOW THIS RELATES TO REPO-SPECIFIC GHA WORKFLOWS.
        #  e.g. in GHA HEAD with be GitHub's merge commit and merge-base will be
        #   the merge target. Performance shifts reported in a PR comment.
    )
    epilog = (
...
def main():
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            ...
            "of this."
            # TEMPLATE NOTE: IRIS-TEST-DATA MAY NOT APPLY TO ALL REPOSITORIES.
    ...
    # NOT TEMPLATED. POPULATE THIS TUPLE AS APPROPRIATE FOR EACH REPO.
    parser_generators: tuple[type(_SubParserGenerator), ...] = (Custom,)
